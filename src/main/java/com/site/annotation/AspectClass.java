...
public class AspectClass {
    ...
    @After(value = "execution(public * com.site.*.*())")
    public void checkAfter(JoinPoint joinPoint) {
        System.out.println("@After " +  " " + joinPoint.getSignature().getName());
    }

    @Around(value = "within(com.site.*)")
    public void checkAround(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    @AfterThrowing(value = "within(com.site.*)", throwing = "error")
    public void printAfterThrowing(Throwable error) {
        System.out.println(LocalTime.now() + " " + error.toString());
    }

    @AfterReturning(value = "within(com.site.*)", returning = "returningValue")
